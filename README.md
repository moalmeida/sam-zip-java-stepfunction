...
```bash
sam-zip-java-stepfunction$ sam build
```
The SAM CLI installs dependencies defined in `functions/<function-name>/pom.xml`, creates a deployment package, and saves it in the `.aws-sam/build` folder.
...
```bash
sam-zip-java-stepfunction$ sam logs -n StockCheckerFunction --stack-name sam-zip-java-stepfunction --tail
```
...
```bash
sam-zip-java-stepfunction$ cd functions/<function-name>/
<function-name>$ mvn test
```
...
```bash
aws cloudformation delete-stack --stack-name sam-zip-java-stepfunction
```
